...
async function list_check(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp) {
    ...
    for (let i = 0; i < pk_id.length; i++) {
        ...
        arr_auth = response[0];
        [logmsg, hdrlog] = await srpr_check(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i);
...
async function srpr_check(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i) {
    const serproid      = arr_temp[0].serproid;
    // hdrlog = prepare(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i);
    // if ( serproid == 1 ) {
    //     hdrlog = googl_send(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i);
    // } else if ( serproid == 2 ) {
    //     hdrlog = micro_send(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i);
    // } else if ( serproid == 6 ) {
    //     hdrlog = smtp_send(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i);
    // } else {
    //     hdrlog = { status: "failed", code: 0, message: `Invalid Account.`, details: `Email integration for this account does not exist.` };
    // }
    logmsg = { status: "success", code: 1, message: "Sent successfully.", details: "All okay." };
    return [logmsg, hdrlog];
}
...
async function prepare(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i) {
    ...
    [ValTo, InvTo, ValCc, InvCc, ValBcc, InvBcc] = emltoccbcc(arr_temp, arr_auth[i]);
    [EmlSubj, EmlBody] = man_rep_hd(emltmpl, EmlSubj, EmlBody, pk_id[i]);
    [EmlSubj, EmlBody] = aut_rep_hd(gbl_hd, arr_auth[i], EmlSubj, EmlBody);
...
async function emltoccbcc(arr_temp, arr_auth) {
    ...
    if ( ToFlag == 1 ) {
        [ValTo, InvTo]   = valtoccbcc(SecTo);
    } else if ( ToFlag == 2 ) {
        [ValTo, InvTo]   = valtoccbcc(arr_auth[1]);
    } else if ( ToFlag == 3 ) {
        [ValTo, InvTo]   = valtoccbcc(ManTo);
    }

    [ValCc, InvCc] = valtoccbcc(`${SecCc},${arr_auth[2]},${ManCc}`);
    [ValBcc, InvBcc] = valtoccbcc(`${SecBcc},${arr_auth[3]},${ManBcc}`);
...
async function googl_send(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i) {

}

async function micro_send(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i) {

}

async function smtp_send(pk_id, usr_login, logmsg, hdrlog, emltmpl, arr_temp, arr_auth, i) {
