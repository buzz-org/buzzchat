...
class OAuthManager {
    ...
    initializeWebSocket() {
        ...
        this.ws.onmessage = (event) => {
            ...
            if (response.status === 'completed' && response.phpOutput?.signupoptions) {
                this.handleSignupTokenResponse(response.phpOutput.signupoptions);
            } else if (response.status === 'completed' && response.phpOutput?.exchangeauth) {
                this.handleExchangeAuthResponse(response.phpOutput.exchangeauth);
            }
        };
    ...
    handleSignupTokenResponse(signupOptions) {
        if (signupOptions.status === 'success') {
            this.clientConfig = {
                clientId: signupOptions.clientid,
                tenantId: signupOptions.tenantid,
                redirectUri: signupOptions.redirect[0]
            };
